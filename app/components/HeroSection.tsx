...
export default function HeroSection({
  ...
  return (
    <section className={`relative ${heights[size]} overflow-hidden bg-white`}>
        ...
        <div className={`max-w-3xl ${centered ? "mx-auto text-center" : ""}`}>
          {subtitle && (
            <div className="inline-flex items-center gap-3 mb-6">
              <span className="w-12 h-px bg-[var(--red)]" />
              <span className="text-[var(--red)] font-semibold text-sm uppercase tracking-[0.15em]">
                {subtitle}
              </span>
              <span className="w-12 h-px bg-[var(--red)]" />
            </div>
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-[var(--font-serif)] text-[var(--charcoal-dark)] mb-6 leading-[1.1] tracking-tight">
            {title}
          </h1>

          {description && (
            <p className="text-lg sm:text-xl text-[var(--gray)] mb-10 leading-relaxed max-w-2xl mx-auto">
              {description}
            </p>
          )}
